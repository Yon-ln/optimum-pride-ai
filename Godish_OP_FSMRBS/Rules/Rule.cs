...
public class Rule_OP
{
    ...
    public Type CheckRule(Dictionary<string, bool> stats)
    {
        ...
        switch (compare)
        {
            ...
            case Predicate.And:
                if (antecendantBools.First().All(x => x == true) && antecendantBools.Last().All(x => x == true))
                { // if antecendant contains any false values, then it is true. then doing !true will make it false.
